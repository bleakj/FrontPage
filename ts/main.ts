...
function createWindow(title: string, windowTitle: string | undefined, content: string, isChild: boolean = false): string {
    ...
    if (title === "3D Prints Gallery - My Stuff") return content;
    return `
        <div class="container">
            <div class="title-bar">
...
function handleNavigation(hash: string): void {
...
function createVCRIntro(): string {
    return `
...
function startClockFlash(): void {
    setInterval(flashClock, 5000);
}

function playVCRAnimation(): Promise<void> {
...
async function startStarWipe(): Promise<void> {
    return new Promise((resolve) => {
...
async function runIntroSequence(): Promise<void> {
...
function getDarkModeIcon(isDark: boolean): string {
    if (isDark) {
